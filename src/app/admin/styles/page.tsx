...
export default function AdminStylesPage() {
  const [items, setItems] = useState<Style[] | null>(null);
  ...
  const handleAdd = async () => {
    ...
    setError(null);
    const supabase = createClient();
    const { error: dbError } = await supabase.from("styles").insert({
      name: form.name,
      slug: form.slug,
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="font-display text-display-md text-espresso">Styles</h1>
        <button
          onClick={() => {
            setShowAdd(true);
            setEditingId(null);
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex items-end pb-2">
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="style-active" className="text-sm text-espresso">
                  Active
                </label>
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        {items === null ? (
          ...
          <div className="p-8 text-center text-umber">No styles yet.</div>
        ) : (
